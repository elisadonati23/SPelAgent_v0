# Parameters for the sardine DEB model
# These parameters are based on the DEB theory and literature values for sardines
...
Tc = exp.(8000.0 ./ 293.0 .- 8000.0 ./ (Temp .+ 273.0)) # Temperature correction factor Ta = 8000, 293 = Tr; 273 conversion to Kelv
Hp = 4553.63
p_Am = 554.351
p_M = 438.602

M0 = M0y / 365.0 # Convert annual mortalities to daily
M1 = M1y / 365.0
M2 = M2y / 365.0
M3 = M3y / 365.0
M4 = M4y / 365.0
...
model_parameters = Dict(
    ...
    :L50 => fit_selectivity([7.98, 15.8, 19.64, 21.7, 23.3], [0.04, 0.42, 1.47, 2.48, 2.49])[:L50],
    :slope_sel => fit_selectivity([7.98, 15.8, 19.64, 21.7, 23.3], [0.04, 0.42, 1.47, 2.48, 2.49])[:slope],
    :Wv => Wv,
    ...
    :spawn_period => days_between_dates(270.0, 90.0),
    :Sex_ratio => 0.5,
    :p_Am => 554.351,
    :v_rate => 0.0216466,
    :Kappa => Kappa,
