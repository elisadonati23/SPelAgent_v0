...
#   Wraps     #
###############
function parallel_eggmass_step!(Anchovy, model)
    eggDEB!(Anchovy, model)
    eggaging!(Anchovy, model)
    egghatch!(Anchovy, model) # egghatch non comporta più un generate_fx() con i superindividui quindi può andare in paralelo
end

function parallel_juvenile_step!(Anchovy, model)
    juvedie!(Anchovy, model)
    juveDEB!(Anchovy, model)
    juvemature!(Anchovy,model)
    juveaging!(Anchovy, model)
end
...
function parallel_anchovy_step!(Anchovy, model)
    if Anchovy.type == :eggmass
        parallel_eggmass_step!(Anchovy, model)  # DEB + aging + hatch
    elseif Anchovy.type == :juvenile
        parallel_juvenile_step!(Anchovy, model)  # Die + DEB + mature + aging
    elseif Anchovy.type == :adult
        parallel_adult_step!(Anchovy, model)    # Die + DEB + aging 
    end
end
...
function complex_step!(model)
    ...
    # Parallel processing for Anchovy agents
    Threads.@threads for anchovy in collect(values(allagents(model)))
        parallel_anchovy_step!(anchovy, model)
    end 
    ...
    for anchovy in adult_ids
        adultspawn!(model[anchovy], model) 
    end
        ...
        if !model.inheritance 
            ...
            # INHERITANCE
                ...
                if model.zoom_factor
                    ...
                    K_value = model.Kappa_value 
                    pM_value = rand(abmrng(model),Normal(mean(pM_values), sqrt(((2)^2)/2)))
                ...
                Hp_value = rand(abmrng(model),Normal(mean_Hp, sqrt((30^2)/2))) # the initial variance divided by 2
                pAm_value = rand(abmrng(model),Normal(mean_pAm_i, sqrt((0.5^2)/2)))#sqrt((0.5^2)/2)))
                pM_value = rand(abmrng(model),Normal(mean_pM, sqrt((2^2)/2)))
                K_value = model.Kappa_value
            end
        end
